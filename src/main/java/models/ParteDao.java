...
public class ParteDao implements DaoInterface<Parte> {
	...
	public List<Parte> getParteByEmployeeId(Empleado empleado) {

		List<Parte> partesList = new ArrayList<>();
		String sql = "SELECT * FROM partes WHERE empleado = ?";

		System.out.println("Empleado NIf en PartesDao =" + empleado.getNif());
	...
	public List<Parte> getAll() {
		...
		try {
			...
			while (rs.next()) {
				/*
				 * Parte parte = new Parte(); parte.setId(rs.getInt("id")); parte = get(parte);
